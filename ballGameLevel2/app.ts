...
let timer: Stroke = new Stroke(1000, 20, "30px arial", "time: ", 300);

let extraJump: Stroke = new Stroke(1000,55,"20px arial","you got extra jump:",30,200,50,"green",1000,33);

let dragon: Dragon = new Dragon(25, 1000, 500, 2, ["red", "green", "yellow", "blue", "black", "red", "green", "yellow", "blue", "black", "red", "green", "yellow", "blue", "black", "red", "green", "yellow", "blue", "black", "red", "green", "yellow", "blue", "black"], 40, 550, true, false, 50, 1100, true, false);

let man: Man = new Man(600,450,200,200);

let ball: Ball = new Ball(100, 589, 15, "blue");

ball.dragon = dragon;

let gravityBall: GravityAttraction = new GravityAttraction(785,270,"yellow",10,1);

ball.boundry = new Array<Baondries>(21);

let lines: Line[] = new Array<Line>(2);
...
ball.lines = new Array<Line>(lines.length);

for (var i: number = 0; i < lines.length; i++) {
	ball.lines = lines;
}

ball.waves = new Array<Baondries>(60);

var id;

let ballenemy: BallEnemy = new BallEnemy(150, 140, 20, 0);

let ballShoot: BallShoot[] = new Array<BallShoot>(3);

let fireWorks: FireWorks[] = new Array<FireWorks>(ballShoot.length);

let bonusBallFireWorks: FireWorks = new FireWorks(10, 10, 5, 10);

for (var i: number = 0; i < bonusBallFireWorks.x.length; i++) {
...
for (var i: number = 0; i < fireWorks.length;i++) {
	 fireWorks[i] = new FireWorks(10, 5, 10,50);
}

for (var i: number = 0; i < ballShoot.length; i++) {
	ballShoot[i] = new BallShoot(ballenemy.x, ballenemy.y, 10, "red");
}



let ballShootArr: BallShoot[] = [];


var moveBaondryToLeft: boolean = false;
var moveBaondryToRight: boolean = true;

let baondries: Baondries[] = new Array<Baondries>(ball.boundry.length);
...
let waves: Baondries[] = new Array<Baondries>(ball.waves.length);
...
for (var n: number = 0; n < baondries.length; n++) {
	ball.boundry[n] = baondries[n];
}

for (var n: number = 0; n < waves.length; n++) {
...
function renderLevel1() {
	...
	for (var i: number = 0; i < baondries.length; i++) {
		...
		if (moveBaondryToRight) { ball.boundry[i].x++; }
		if (moveBaondryToLeft) { ball.boundry[i].x--;}
	}
	...
	for (var i: number = 0; i < ballShootArr.length; i++) {
		...
		for (var i: number = 0; i < ballShootArr.length; i++) {
			...
			if (ballShootArr[i].y >= canvas.height - 30) {
				...
				for (var f: number = 0; f < fireWorks[i].x.length; f++) {
	...
	for (var i: number = -20; i <= 20; i ++) {
		for (var y: number = -20; y <= 20; y++) {
	...
	for (var index: number = 0; index < ballShootArr.length; index++) {

		Math.round(ballShootArr[index].x);
		Math.round(ballShootArr[index].y);
		for (var i: number = -ball.radios; i < ball.radios; i ++) {
			for (var r: number = -ball.radios; r < ball.radios; r ++) {
	...
	for (var x: number = -gravityBall.radios - ball.radios; x < gravityBall.radios + ball.radios; x++) {

			if (!brk) {

				if (Math.round(ball.x) + x == gravityBall.x) {
					for (var y: number = -gravityBall.radios - ball.radios; y < gravityBall.radios + ball.radios; y++) {
